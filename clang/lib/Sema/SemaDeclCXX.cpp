...
Sema::BuildMemInitializer(Decl *ConstructorD,
  ...
  if (TemplateTypeTy) {
    ...
    BaseType = ActOnPackIndexingType(DS.getRepAsType().get(), DS.getPackIndexingExpr(), DS.getBeginLoc(), DS.getEllipsisLoc());
  } else {
...
bool Sema::EvaluateAsString(Expr *Message, std::string &Result, ASTContext &Ctx){
  ...
  auto BuildExpr = [&](LookupResult & LR) {
    ...
    if (Res.isInvalid())
      return ExprError();
    Res = BuildCallExpr(nullptr, Res.get(), Loc, std::nullopt, Loc, nullptr, false, true);
    if (Res.isInvalid())
      return ExprError();
