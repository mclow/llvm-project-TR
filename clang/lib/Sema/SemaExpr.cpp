...
Sema::ActOnArraySubscriptExpr(Scope *S, Expr *base, SourceLocation lbLoc,
  ...
  if (matSubscriptE) {
    ...
    return CreateBuiltinMatrixSubscriptExpr(
        matSubscriptE->getBase(), matSubscriptE->getRowIdx(), ArgExprs.front(), rbLoc);
  }
  ...
  if (base->getType()->isMatrixType()) {
    ...
    return CreateBuiltinMatrixSubscriptExpr(base, ArgExprs.front(), nullptr, rbLoc);
  }

  if (ArgExprs.front()->getType()->isNonOverloadPlaceholderType()) {
    ExprResult result = CheckPlaceholderExpr(ArgExprs.front());
    if (result.isInvalid()) return ExprError();
